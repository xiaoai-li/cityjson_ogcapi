...
jindex={'collections':query_collections("test")}
...
def items(dataset):
    ...
    if re_f == 'html' or re_f is None:
        return render_template("items.html", datasetname=dataset,jcm=cm.j, limit=re_limit, offset=0)
    elif re_f == 'json':
        return cm.j
    else:
        return JINVALIDFORMAT
...
def item(dataset, featureID):
    re = request.args.get('f', None)
    cm = query_feature(dataset, "test", featureID)
